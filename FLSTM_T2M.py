...
# 1. 数据集：气象数据
def generate_weather_data(n_days=1460):
    # 模拟季节性（年周期）+ 周期扰动 + 气象因素
    days = np.arange(n_days)
    date = pd.date_range("2018-01-01", periods=n_days, freq="D")
    day_of_year = (date.dayofyear.values).astype(float)

    # 年周期温度基线 (摄氏度)
    temp_base = 15 + 10 * np.sin(2 * np.pi * day_of_year / 365.25)  # 春夏秋冬

    # 周期性扰动（周波动）
    weekly = 2 * np.sin(2 * np.pi * days / 7)

    # 随机噪声
    noise = np.random.normal(0, 1.5, size=n_days)

    # 其他气象变量（与温度相关，且有各自的结构）
    humidity = 60 + 20 * np.sin(2 * np.pi * day_of_year / 365.25 + 0.5) + np.random.normal(0, 5, size=n_days)
    pressure = 1013 + 5 * np.cos(2 * np.pi * day_of_year / 365.25) + np.random.normal(0, 2, size=n_days)
    wind = 3 + 1.5 * np.abs(np.sin(2 * np.pi * days / 10)) + np.random.normal(0, 0.5, size=n_days)
    rainfall = np.maximum(0, np.random.gamma(shape=2.0, scale=1.5, size=n_days) - 2.0)  # 偶尔有雨
    cloud = 50 + 30 * np.abs(np.sin(2 * np.pi * day_of_year / 365.25 + 1.0)) + np.random.normal(0, 10, size=n_days)

    temp = temp_base + weekly + 0.05 * humidity - 0.02 * pressure - 0.5 * wind - 0.1 * rainfall + 0.03 * cloud + noise
...
class SimpleCascadeForestRegressor:
    ...
    用验证集MSE改善来早停。
    """
    def __init__(self, n_layers_max=3, random_state=42,rf_params=None, et_params=None, tol=1e-4):
...
df = generate_weather_data(n_days=1460)
df = add_lag_features(df, vars_to_lag=["temp","humidity","pressure","wind","rainfall","cloud"], max_lag=7)
...
plt.title("图4：深度森林特征重要性（Top 15）")
plt.xlabel("重要性(平均)")
plt.ylabel("特征名")
plt.tight_layout()
plt.show()

# 图5：残差自相关（ACF）图（手工计算滞后相关）
def acf(x, nlags=30):
    x = x - np.mean(x)
    acf_vals = [1.0]
    var = np.dot(x, x)
